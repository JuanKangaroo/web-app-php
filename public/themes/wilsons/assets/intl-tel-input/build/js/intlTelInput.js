...
(function(factory) {
    ...
    defaults = {
        //show country code instead of flag COUNTRY_CODE, FLAG
        showSelectedItem: "COUNTRY_CODE",
        //move to the right second dropdown
        secondDropdownLeft: '0',
        //show country abreviation on the right
        showCountryAbr:true,
        //show delete input content icon on the right
...
//======================================  Extended code  ======================================
            if(this.options.clearInputIcon){
                this.clearInputContainer = $("<div>", {
                    "class": "clear-input"
...
//======================================  Extended code  ======================================
            //country code on the left instead of flag
            this.selectedCodeInner = $("<div>", {
                "class": "iti-country-code"
            }).appendTo(selectedFlag);
            
            this.selectedCodeInner.html('+1');

            //country short name on the right
            var countryAbrHtml = "";
            if(this.options.showCountryAbr){
                countryAbrHtml = "<div class='intl-tel-country-abr' id='intl-tel-country-abr'>" + this.options.defaultCountry.toUpperCase();
                countryAbrHtml += "</div>";
            }

            this.countryAbr = $(countryAbrHtml, {
            }).appendTo('.intl-tel-input');
            
            //move to the right
            this.countryAbr.css('right','0').css('left','initial');

            if(this.isMobile){
                //prepare select item for country list
                this.countryDropdown = $("<select>", {
                    "id": "intl-tel-country-select",
                    "class": "intl-tel-country-select",
                }).appendTo('.intl-tel-input');
                
                this.countryDropdown.css('right','0').css('left','initial');

                //create country list
                var tmp = '';
                $.each(this.countries, function(i, country) {    
                    tmp += "<option data-dial-code='" + country.dialCode + "' value='" + country.iso2 + "'>";
                    tmp += country.name + " +" + country.dialCode;
                    tmp += "</option>";
                });

                this.countryDropdown.append(tmp);
            }
            ...
            if (this.isMobile) {
                ...
                selectedFlag.on("click" + this.ns, function(e) {
                    ...
                    if (that.countryList.hasClass("hide") && !that.telInput.prop("disabled") && !that.telInput.prop("readonly")) {
                        that._showDropdown();
                        that.countryList.css('left','0px');
                    }
                });

//============================ Extended Open the dropdown when the country abbreviation is clicked
                this.countryAbr.on("click" + this.ns, function(e) {
                    // only intercept this event if we're opening the dropdown
                    // else let it bubble up to the top ("click-off-to-close" listener)
                    // we cannot just stopPropagation as it may be needed to close another instance
                    if (that.countryList.hasClass("hide") && !that.telInput.prop("disabled") && !that.telInput.prop("readonly")) {
                        that._showDropdown();
                        that.countryList.css('left',that.options.secondDropdownLeft+'px');
                    }
                });
//============================  Open the dropdown when the country abbreviation is clicked
            }
        ...
        _selectFlag: function(countryCode, updateDefault) {
            ...
            //show country code or flag
            if (this.options.showSelectedItem=="COUNTRY_CODE") {
                this.selectedCodeInner.html("+"+this.selectedCountryData.dialCode);
                this.selectedFlagInner.hide();
            } else {
                this.selectedCodeInner.hide();
            }
